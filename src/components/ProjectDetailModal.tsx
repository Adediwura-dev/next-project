...
import { getProjectDetails, ProjectGuide } from "@/lib/ai-service.ts";
...
export function ProjectDetailModal({ isOpen, onClose, project }: ProjectDetailModalProps) {
    const [guidesCache, setGuidesCache] = useState<Record<string, ProjectGuide>>({});
    const [loading, setLoading] = useState(false);

    const currentGuide = project ? guidesCache[project.title] : undefined;

    useEffect(() => {
        if (isOpen && project) {
            if (guidesCache[project.title]) {
                setLoading(false);
                return;
            }
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-2xl h-[85vh] p-0 flex flex-col bg-card/95 backdrop-blur-xl border-border/50 overflow-hidden">
                ...
                <DialogHeader className="p-6 border-b border-border/40 shrink-0">
                    ...
                    <DialogDescription>
                        {loading && !currentGuide ? "Asking AI for the best way to start..." : currentGuide?.introduction}
                    </DialogDescription>
                </DialogHeader>
                ...
                <ScrollArea className="flex-1 min-h-0 p-6 pt-2">
                    {loading && !currentGuide ? (
                        <div className="flex flex-col items-center justify-center h-64 space-y-4">
                            ...
                            <p className="text-muted-foreground animate-pulse">Generating your custom guide...</p>
                        </div>
                    ) : currentGuide ? (
                        <div className="space-y-8 pb-8">
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {currentGuide.youtubeQueries.map((query, idx) => (
                                        <a
                                            key={idx}
